...
public class Summary {
	...
	/* 默认IK分词器 */
	private final static String DEFAULT_SEGMENTER_SERVICE = "IK";
	...
	public Summary() {
		...
		this.tokenServiceName = DEFAULT_SEGMENTER_SERVICE;
	}
	...
	public String execute(String text, Method method) throws IOException {
		...
		if("jieba".equals(tokenServiceName))
			segments = jiebaSegment(text);
		else
			segments = ikSegment(text);
		/* 对各分词分类统计并按照频数降序 */
	...
	private List<String> ikSegment(String text) throws IOException {
		...
		String word = null;
		SegToken segToken = null;
		List<SegToken> tokens = segmenter.process(text, SegMode.SEARCH);
		for(int i = 0, size = tokens.size(); i < size; i++) {
			segToken = tokens.get(i);
			word = segToken.word;
			if(word.isEmpty())
				continue;
			if(word.equals("\\s") || word.equals("\r") || word.equals("\n") || word.equals("\t"))
				continue;
