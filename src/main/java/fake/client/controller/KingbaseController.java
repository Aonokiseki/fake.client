...
public class KingbaseController {
	...
	private JdbcTemplate kingbaseTemplate;
	
	@Autowired
	private Gson gson;
	
	@ApiOperation(value = "SQL查询", notes="根据所给SQL语句查询, 返回列表")
	@GetMapping(path = "/kingbase/query")
	@ApiImplicitParams({
	...
	public String query(String sql) {
		...
		List<Map<String, Object>> result = kingbaseTemplate.queryForList(sql);
	...
	@PostMapping(path = "/kingbase/update")
	@ApiImplicitParams({
		@ApiImplicitParam(paramType = "query", dataTypeClass = String.class, name = "sql", value = "SQL语句", required = true),
	})
	@ApiResponses({
	...
	public String update(String sql) {
		...
		int result = kingbaseTemplate.update(sql);
	...
	@PostMapping(path = "/kingbase/execute")
	@ApiImplicitParams({
		@ApiImplicitParam(paramType = "query", dataTypeClass = String.class, name = "sql", value = "SQL语句", required = true)
	})
	@ApiResponses({
